...
def gather_energies():
    ...
    for molname, bond_lengths in molecules.items():
        ...
        for bond_length in tqdm(bond_lengths):
            ...
            hf_energy = get_energies(molname, bond_length, charge)
            hf_energies.append(hf_energy)
        hf_results[molname] = hf_energies

    # Saving results to npy files
    for molname in molecules.keys():
        np.save(f'results/hf_energies_{molname}.npy', np.array(hf_results[molname]))

    return hf_results

if __name__ == "__main__":
    hf_results = gather_energies()
    print("HF energies have been calculated and saved.")
